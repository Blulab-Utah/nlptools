...
import edu.utah.bmi.simple.gui.task.FastDebugPipeSampleSize;
 ...
 */
public class RunPipeDebugger extends GUITask {

	private TasksFX tasks;
	protected boolean fastNerCaseSensitive;
	protected static ClinicPipe fastDebugPipe;
	protected GUITask guiTask;
	private String inputStr, metaStr;


	public RunPipeDebugger(TasksFX tasks) {
		initiate(tasks);
	}
		...
		if (guiEnabled)
			Platform.runLater(() -> {
				...
				if (entered.trim().length() > 0) {
					//                        initiate(tasks, "xmi");
					updateGUIMessage("Wait till ready...");
					new Thread(() -> {
						fastDebugPipe = ClinicPipe.getInstance(tasks, "debug");
