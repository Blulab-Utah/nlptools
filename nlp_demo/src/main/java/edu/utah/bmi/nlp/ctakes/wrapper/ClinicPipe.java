...
import edu.utah.bmi.nlp.uima.AdaptableUIMACPEJCasRunner;
 ...
 */
public class ClinicPipe {
	private static Logger logger = IOUtil.getLogger(ClinicPipe.class);
	protected static HashMap<String, ClinicPipe> clinicPipes = new HashMap<>();
	...
	public static ClinicPipe getInstance(TasksFX tasks) {
		return getInstance(tasks, "debug");
	}


	public static ClinicPipe getInstance(TasksFX tasks, String option) {
		ClinicPipe clinicPipe;
		if (!clinicPipes.containsKey(option)) {
			clinicPipe = new ClinicPipe(tasks, option);
			clinicPipes.put(option, clinicPipe);
		} else {
			clinicPipe = clinicPipes.get(option);
		}
		return clinicPipe;
	}

	public static ClinicPipe getInstance(String configFile) {
		return getInstance(configFile, "debug");
	}

	public static ClinicPipe getInstance(String configFile, String option) {
		ClinicPipe clinicPipe;
		if (!clinicPipes.containsKey(option)) {
			clinicPipe = new ClinicPipe(configFile, option);
			clinicPipes.put(option, clinicPipe);
		} else {
			clinicPipe = clinicPipes.get(option);
		}
	...
	public ClinicPipe(String configFile, String option) {
		setOption(option);
	...
	public ClinicPipe(TasksFX tasks) {
		init(tasks);
	}

	public ClinicPipe(TasksFX tasks, String option) {
	...
	protected void initiate(TasksFX tasks, String option) {
		...
		String rawStringValue = config.getValue(ConfigKeys.reportPreannotating);
